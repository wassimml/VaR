...
def main():
    ...
    fig.suptitle("Effet du Kurtosis sur l'épaisseur des queues de distribution",
    ...
    for ax, (dist, title, color, note) in zip(axes, configs):
        ...
        ax.plot(x, normal, color="gray", linewidth=1.2, linestyle="--", alpha=0.6,
                label="Normale (référence)")
    ...
    fig.savefig(REPORTS_DIR / "kurtosis_illustration.png", dpi=150, bbox_inches="tight")
    plt.show()
