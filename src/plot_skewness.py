...
def main():
    ...
    fig.suptitle("Effet du Skewness sur la distribution des rendements",
    ...
    for ax, (dist, title, color, note) in zip(axes, configs):
        ...
        ax.fill_between(x, dist, where=(x < -1.5), alpha=0.5, color="#e74c3c",
                        label="Zone pertes extrêmes")
    ...
    fig.savefig(REPORTS_DIR / "skewness_illustration.png", dpi=150, bbox_inches="tight")
    plt.show()
